...
class displaCy {
  constructor(api, options) {
    ...
    this.collapsePunct =
      options.collapsePunct != undefined ? options.collapsePunct : true
    this.collapsePhrase =
      options.collapsePhrase != undefined ? options.collapsePhrase : true
  ...
  handleConversion(parse) {
    switch (this.format) {
      ...
      case 'google':
        return {
          words: parse.map(
            ({ text: { content: text }, partOfSpeech: { tag } }) => ({
              text,
              tag,
            })
          ),
