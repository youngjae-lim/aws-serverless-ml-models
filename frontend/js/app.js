...
function sendResNetPredictionRequest(photoKey) {
  return $.ajax({
    ...
    error: function () {
      $('#chartLoader').hide()
      alert(
        'There was an error while sending prediction request to ResNet50 model.'
      )
    },
  })
}
...
function sendInceptionV3PredictionRequest(photoKey) {
  ...
  return $.ajax({
    ...
    error: function () {
      $('#chartLoader').hide()
      alert(
        'There was an error while sending prediction request to Inception V3 model.'
      )
    },
  })
}
...
function sendImagePredictionRequestsAndPlotImagePredictionsChart(photoKey) {
  $.when(
    sendResNetPredictionRequest(photoKey),
    sendInceptionV3PredictionRequest(photoKey)
  ).done(function (resnet50Response, inceptionV3Response) {
    ...
    for (let i = 0; i < resnetPredictions.length; i++) {
      ...
      if (inceptionV3Response != false) {
        ...
        inceptionProbability =
          inceptionV3Response[0]['predictions'][i]['probability']
...
function sendHousePredictionRequest() {
  ...
  $.ajax({
    ...
    error: function (error) {
      alert(
        'Error while obtaining house price prediction. Please check all fields and try again.'
      )
    },
